...
sim_season <- function(
  ...
  for (i in 1:nrow(yesterday_df)) {
    this_row <- yesterday_df[i, ]
    this_stat <- hist_comps[hist_comps$stat_name == this_row$stat_name, ]
    
    #sample n times
    stat_samples <- this_stat %>%
      dplyr::group_by(team_key) %>%
      dplyr::sample_n(size = n_walks)
    
    stat_samples$sim_mean_walk <- stat_samples$mean_delta + this_row$mean_21
    stat_samples$sim_sd_walk <- stat_samples$sd_delta + this_row$sd_21
