...
for (i in hpk_hist$year %>% unique() %>% sort()) {
  ...
  max_day <- max(this_year$clean_day_of_season)
  eoy_stats <- this_year[this_year$clean_day_of_season == max_day, ] %>%
    dplyr::select(team_key, stat_name, running_total)
  names(eoy_stats)[3] <- 'final_value'
  this_year <- this_year %>% 
...
#     if (this_date$day_of_season %>% unique() < 30) {
#       next
#     }  
    ...
    this_date_all$final_predicted_all <- max_day * this_date_all$slope_all + 
    ...
    this_date_30$final_predicted_30 <- max_day * this_date_30$slope_30 + 
