...
for (i in c('R', 'RBI', 'SB', 'TB')) {
  clean_asb <- hpk_clean[hpk_clean$stat_name == 'AB' & hpk_clean$value > 0, 'day_of_season'] %>%
    unique()
  ...
  for (j in unq_teams) {
    ...
    bb_linreg <- bayes_boot(
      this_team, 
      function(data) {
        lm(running_total ~ day_of_season, data)$coef
      }, 
      n1 = 1000
    )
    
    names(bb_linreg) <- c('intercept', 'slope')
    bb_linreg$team_key <- j
    bb_linreg$stat_name <- i
    bb_linreg$end_value <- bb_linreg$slope * 177 + bb_linreg$intercept
        
    bb_data <- rbind(bb_data, bb_linreg)
  }
  #add owner
  bb_data <- bb_data %>%
    dplyr::left_join(short_names)
